...
(function(self) {
var VERSION = '1.0.2';
var longname_SWITCH_RE = /^--\w/;
var shortname_SWITCH_RE = /^-\w/;
    ...
    for(var i=0; i<arr.length; i++) {
        ...
        switch(r.length) {
            case 1:
                ...
                break;
            case 2:
                var expr = longname_SWITCH_RE(r[0]) ? 0 : 1;
...
//  Builds a rule with specified expression, shortname style switch and help. This 
//  function expects a dict with filters to work correctly. 
//
//  Return format:
//      name               The name of the switch.
//      shortname              The shortname style switch
//      longname               The longname style switch
...
function contains_expr(arr) {
    ...
    var l = arr.length;
    while(l-- > 0) if(longname_SWITCH_RE(arr[l])) return true;
    return false;
}
...
OptionParser.prototype = {
    ...
    parse: function(args) {
        ...
        while((token = tokens.shift()) && this._halt == false) {
            if(longname_SWITCH_RE(token) || shortname_SWITCH_RE(token)) {
                var arg = undefined;
                // The token is a longname or a shortname switch. Get the corresponding 
                // rule, filter and handle it. Pass the switch to the default 
                // handler if no rule matched.
                for(var i = 0; i < rules.length; i++) {
                    var rule = rules[i];
                    if(rule.longname == token || rule.shortname == token) {
                        if(rule.filter !== undefined) {
                            arg = tokens.shift();
                            if(!longname_SWITCH_RE(arg) && !shortname_SWITCH_RE(arg)) {
                                try {
                                    arg = rule.filter(arg);
                                } catch(e) {
                ...
                // Did not match longname or shortname switch. Parse the token as a 
                // normal argument.
    ...
    toString: function() {
        var builder = [this.banner, '', this.options_title], 
            shortnames = false, longnameest = 0, rule;
        var rules = build_rules(this.filters, this._rules);
        for(var i = 0; i < rules.length; i++) {
            rule = rules[i];
            // Quick-analyze the options. 
            if(rule.shortname) shortnames = true;
            if(rule.decl.length > longnameest) longnameest = rule.decl.length;
        }
        for(var i = 0; i < rules.length; i++) {
            var text; 
            rule = rules[i];
            if(shortnames) {
                if(rule.shortname) text = spaces(2) + rule.shortname + ', ';
                else text = spaces(6);
            }
            text += spaces(rule.decl, longnameest) + spaces(3);
            text += rule.desc;
            builder.push(text);
        }
        return builder.join('\n');
