...
exports.State = {
	...
	onUpdate: function(server, conn, data){
		...
		server.$state[data.key] = data.value;
	...
	onAcquireLock: function(server, conn, key){
		...
		if (server.$timeouts[key]) clearTimeout(server.$timeouts[key]);
		this.acquireLock(server, conn, key);
		server.$timeouts[key] = setTimeout(this.releaseLock.bind(this, server, conn, key), 20000);
		return this;
	},
	...
	onReleaseLock: function(server, conn, key){
		...
		return this.releaseLock(server, conn, key);
	},
