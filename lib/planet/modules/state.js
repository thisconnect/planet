...
exports.State = {
	...
	onClientConnect: function(server, conn){
		...
		conn.on('create', this.onCreate.bind(this, server, conn));
	...
	onStateUpdate: function(server, conn, data){
		...
		return this.pushUpdateError(server, conn, data);
	},

	onCreate: function(server, conn, data){
		for (var i in data){
			//if (data.hasOwnProperty(i)) 
			server.$state[i] = data[i];
		}
		this.send(server, 'create', data);
	},
	...
	pushUpdateError: function(server, conn, data){
		return this.reply(conn, 'update error', data);
	},
