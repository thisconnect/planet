...
var State = exports.State = {
	...
	onClientConnect: function(server, conn){
		conn.on('attempt update', this.onStateUpdate.bind(this, server, conn, true));
		conn.on('state update', this.onStateUpdate.bind(this, server, conn, false));
	...
	onStateUpdate: function(server, conn, bypass, data){
	...
	pushUpdate: function(server, conn, data){
		var message = {};
		message[data.component] = data.payload;
		return this.send(server, 'state update', message);
	},
